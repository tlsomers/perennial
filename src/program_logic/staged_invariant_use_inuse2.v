...
Proof using later_tokG0.
  ...
  rewrite /wpc_pre. iSplit; last first.
  {
    ...
    iApply (lc_fupd2_add_laterN with "Htoklc"). iNext.
  ...
  }
  {
    ...
    iAssert (||={⊤|⊤ ∖ D ∖ E2, ⊤|⊤ ∖ D ∖ E2}=> (_ ∧ _))%I with "[-]" as ">$".
    ...
    { iNext.
      ...
      rewrite /wpc_crash_modality.
      iIntros (???) "Hg #HC".
      replace E1' with (⊤ : coPset) by set_solver.
      iDestruct ("Hwpc" with "[$] [$]") as ">(Hg&(HΦc&Hwpc))".
      iSpecialize ("Hwpc" with "[$] [$]"). by iFrame.
    }
    iDestruct ("Hg_inv_clo" with "Hg") as "$".
...
Lemma wpc_staged_inv_inuse2 E1 e Φ Φc Qs P :
  ...
  ⊢ WPC e @ E1 {{ Φ }} {{ Φc }}.
Proof using later_tokG0.
  iIntros (Hnval) "([Hltok2 Htoklc2]&Hstaged&Hwp)".
  ...
  iDestruct (later_tokN_use with "Hltok") as "[Htoklc Hcl]".
  ...
  iAssert (||={E1|⊤ ∖ D ∖ E2, E1|⊤ ∖ D ∖ E2}=> (_ ∧ _))%I with "[-]" as ">$".
  ...
  iEval (rewrite -Heq_mj) in "Hitok".
  iDestruct (pri_inv_tok_split with "[$Hitok]") as "(Hitok_ukeep&Hitok_ushare)".
  iMod ("Hclo" with "[Hown' Hstatus' H Hitok_ishare Hitok_ushare Hltok2 Htoklc2]").
  { iNext.
    iEval (rewrite staged_inv_inner_unfold).
    iExists _, _, _, _, _. iFrame "∗ #".
    iLeft.
